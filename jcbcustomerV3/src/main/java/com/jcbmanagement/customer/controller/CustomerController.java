...
public class CustomerController {
    ...
    public String customerBookings(Model model) {
        try {
            logger.info("Customer bookings accessed");
            ...
            if (customer != null) {
                ...
                logger.info("Found " + customerBookings.size() + " bookings");
                ...
                for (Booking booking : customerBookings) {
                    try {
                        ...
                        logger.warning("Error processing payment status for booking " + booking.getBookingID() + ": " + e.getMessage());
            ...
            logger.severe("Error in customer bookings: " + e.getMessage());
            e.printStackTrace();
            model.addAttribute("bookings", new ArrayList<>());
            model.addAttribute("paymentStatusMap", new HashMap<>());
            model.addAttribute("title", "My Bookings");
    ...
    public String newBookingFormWithMachine(@PathVariable Long machineId, Model model, RedirectAttributes redirectAttributes) {
        try {
            logger.info("New booking form accessed for machine: " + machineId);
            Optional<Machine> machineOpt = machineService.getMachineById(machineId);
            if (machineOpt.isEmpty()) {
                logger.warning("Machine not found: " + machineId);
            ...
            logger.info("Machine loaded: " + machineOpt.get().getModel());
            return "customer/new-booking-form";
        } catch (Exception e) {
            logger.severe("Error in new booking form: " + e.getMessage());
            e.printStackTrace();
